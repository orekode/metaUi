...
export default function Stripe() {
    ...
    const checkInputs = () => {
        ...
        if(inputs.length < 10 || check_inputs.length < 8) {
            Swal.fire({
                icon: 'error',
                title: 'Empty Inputs',
            ...
            <AnimatePresence>
                {!user_details.next &&
                    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="form-one max-w-[500px] mx-auto my-9 mb-12">
                        <div className="h-[200px] w-[200px] mx-auto rounded-full shadow overflow-hidden relative mb-6">
                            {user_details.url &&
                                <img src={user_details.url} className="object-cover h-full w-full" />
                            }
                            <div onClick={() => inputRef.current.click()} className="absolute top-0 left-0 h-full w-full flex items-center justify-center flex-col">
                                <Upload size={70} />
                                <div className="text-xs">Click To Upload Image</div>
                            </div>
                            <input ref={inputRef} onChange={previewImage} type="file" name="image" accept="image/*" id="" className="h-0 w-0 overflow-hidden" />
                        </div>
                        <div className="grid grid-cols-2 gap-3 mb-3 max-[500px]:grid-cols-1">
                            <div className="input flex flex-col gap-1">
                                <label className="text-sm text-gray-700" htmlFor="first-name">First Name</label>
                                <input onChange={event => set_user_details({...user_details, first_name: event.target.value})}  value={user_details.first_name}  type="text" placeholder="John" name="first-name" className="p-3 rounded-md border shadow" />
                            </div>

                            <div className="input flex flex-col gap-1">
                                <label className="text-sm text-gray-700" htmlFor="last-name">Last Name</label>
                                <input onChange={event => set_user_details({...user_details, last_name: event.target.value})}  value={user_details.last_name}   type="text" placeholder="John" name="last-name" className="p-3 rounded-md border shadow" />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3 mb-3 max-[500px]:grid-cols-1">
                            <div className="input flex flex-col gap-1">
                                <label className="text-sm text-gray-700" htmlFor="email">Email</label>
                                <input onChange={event => set_user_details({...user_details, email: event.target.value})} value={user_details.email}  type="text" placeholder="e.g adedavid@gmail.com" name="email" className="p-3 rounded-md border shadow" />
                            </div>

                            <div className="input flex flex-col gap-1">
                                <label className="text-sm text-gray-700" htmlFor="contact">Contact</label>
                                <input onChange={event => set_user_details({...user_details, contact: event.target.value})}   value={user_details.contact}  type="text" placeholder="09000000000" name="contact" className="p-3 rounded-md border shadow" />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3 mb-3 max-[500px]:grid-cols-1">
                            <div className="input flex flex-col gap-1">
                            ...
                            <div className="input flex flex-col gap-1">
                                <label className="text-sm text-gray-700" htmlFor="email">Payment Type</label>
                                <select onChange={event => set_user_details({...user_details, payment_type: event.target.value})} defaultValue={user_details.payment_type}  type="text" placeholder="e.g adedavid@gmail.com" name="email" className="p-3 rounded-md border shadow bg-white">
                                    <option disabled selected>Select a program</option>
